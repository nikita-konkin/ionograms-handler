...
MESSAGES: dict[str, str] = {
    ...
    "sources.manual_panel": "Добавить источник вручную",
    "sources.manual_note":
        "Перепись предлагает только то, что этот приёмник уже слышал. "
        "Используйте это для передатчика, который сегодня не в эфире, для "
        "того, чьи обнаружения были отброшены как помеха, или для трассы, "
        "которую настраивают до прихода первой развёртки.",
    "sources.field.rate": "Скорость (кГц/с)",
    "sources.field.slots": "Слоты",
    "sources.manual_slots_note":
        "Записывается как секунда:цикл, так же как их показывает пульт "
        "&mdash; <code>210:300</code> это 210-я секунда цикла в 300 с. "
        "Несколько разделяйте запятыми. Секунда указана по <em>приёму</em>: "
        "секунда передачи плюс время пробега в одну сторону плюс смещение "
        "эпохи этого приёмника.",
    "sources.manual_note_placeholder": "Откуда взяты эти числа",
    "sources.manual_save": "Добавить передатчик",
    ...
    "console.storage": "Папка хранения",
    "console.apply_storage": "Перенести хранение",
    "console.storage_hint":
        "Должна находиться внутри папки, из которой копируют архивные задания: "
        "они обходят вложенные каталоги, поэтому подпапка попадёт на NAS и "
        "будет освобождена без изменений на станции. Всё остальное никто не "
        "скопирует и не очистит, и агент это отклонит.",
    ...
    "sources.js.slot_shape":
        "{slot} не является слотом. Пишите секунда:цикл, например 210:300.",
    "sources.js.slot_required": "Нужен хотя бы один слот.",
    "sources.js.rate_required":
        "Нужна скорость свипа в кГц/с, например 50.",
    "sources.js.saved": "Добавлен {code}.",
    ...
    "console.js.enter_storage_first": "Сначала укажите папку хранения.",
    ...
    "console.js.storage_sending": "Отправка \u2026",
    "console.js.storage_queued":
        "Папка хранения поставлена в очередь как {id} \u2014 станция применит "
        "её при следующем опросе, а в силу это вступит при перезапуске приёма, "
        "поэтому после получения нажмите перезапуск. Уже накопленные продукты "
        "останутся на месте: архивные задания обслуживают обе папки, пока "
        "старая не опустеет.",
