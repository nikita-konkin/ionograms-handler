...
def MUF_interpolation(exp_muf_set, year, month, day):
    ...
    min_time = str(min(exp_muf_set.time))
    max_time = str(max(exp_muf_set.time))

    min_time = f'{year}-{month}-{day} {min_time}'
    max_time = f'{year}-{month}-{day} {max_time}'

    date_start = pd.to_datetime(min_time, format='%Y-%m-%d %H:%M:%S')
    date_stop = pd.to_datetime(max_time, format='%Y-%m-%d %H:%M:%S')
...
#     print(true_index)
    ...
    concat[['date', 'time']] = concat['time'].astype(str).str.split(' ', expand=True)
    concat['datetime'] = f'{year}-{month}-{day} ' + concat['time']
    concat['datetime'] = pd.to_datetime(concat['datetime'], format='%Y-%m-%d %H:%M:%S')
    
    concat.index = pd.DatetimeIndex(concat['datetime'])
    concat = concat.drop(['date', 'time', 'datetime'], axis=1)
...
def main():
    ...
    df_total = pd.DataFrame([], columns=['muf'])
    year = 2023
    print('start')
        ...
        for table_name in tables_names:
            ...
            if len(split) == 1:
                ...
                for transmit_station in transmit_stations:
                    ...
                    # interpolated_muf.muf.plot()
                    # plt.plot(interpolated_muf.muf)
                    df_total = pd.concat([df_total, interpolated_muf])

    muf = pd.DataFrame(df_total.muf)
    muf.to_csv(str(schema[0].split('_')[1])+'_'+str(table_name[0].split('_')[0])+'.csv', header=False)
    df_total = df_total.sort_index()
        # plt.plot(df_total.muf)
        # plt.show()
