...
| `0014-rx_uhd_ext_gps-take-the-LO-from-the-command-line.patch` | adds `--center-freq` (default `12.5e6`), and prints the LO and the band it implies on every start | 0012 hardcodes the LO, leaving it in two places — the binary and `center_freq` in the ini — with nothing keeping them equal. Nothing checks them and nothing can: the LO is not in the Digital RF metadata, so a mismatch dechirps by the difference and yields **empty products with no error in any log**. It blinded the station twice on 2026-08-19. The mechanism is upstream's own — `w2naf.sh:29-36` already reads the ini and passes `--gps-lock-timeout`, declared three lines above the option this adds. **Supersedes 0012**; the new `std::cout` is half the value, since both incidents were silent |
| `0015-dombas-read-the-recorder-LO-from-the-ini.patch` | `dombas.sh` reads `center_freq` from the ini and passes `--center-freq`; fatal if the read fails | the launcher half of 0014 — apply both or neither, since 0014 alone defaults to `12.5e6` against an ini saying `20e6`. **`dombas.sh` already had `CENTER_FREQ=12.5e6` and nothing read it**: the same dead assignment is in eight other launchers under `examples/`, including `aeroauto.sh` at `20e6`, because the recorder had no option to accept it until 0014. `dombas.sh` is the rollback path now that systemd is live, which is exactly why it must not tune to 12.5 MHz against a 20 MHz ini |

**The live launcher is systemd, and 0015 does not touch it.** `chirp-rx.service`
runs `tools/chirp-rx-launch.sh` in this repo, which does the same job for the
same reason and is covered by `tests/test_systemd_units.py`. 0015 exists for
`dombas.sh` because that is the rollback path: rolling back onto a recorder
rebuilt with 0014 would otherwise tune to 12.5 MHz against an ini that says 20.
Both launchers end in `exec` so the recorder stays systemd's MAINPID — without
it `KillSignal=SIGINT` stops the shell, and a USRP that never sees SIGINT keeps
transmitting UDP until someone removes power.

**Do not wire `--rate` up the same way.** `sample_rate_numerator` at
`rx_uhd_ext_gps.cpp:173` is a separate hardcoded constant feeding the Digital RF
metadata and the timestamp→index arithmetic, so passing `--rate` alone corrupts
the timebase silently. Every script under `examples/ringbuffer/` does exactly
that.
