    ...
    for schema in schemas:
        ...
        for table_name in tables_names:
            ...
            if len(split) > 1 and split[1] == 'interp':
                ...
                if len(interp_time_muf) > 27:

                    y, x, y_filtered_savgol = savgol1_filter(interp_time_muf)
                    y_filtered_savgol = pd.DataFrame({'filtered_savgol_MUF': y_filtered_savgol})
                    ...
                    # if len(y_filtered_savgol) == 288 and check_db_minimuf_sunspots_table_exist(D6, schema[0], transmit_station) == False:
                    if len(y_filtered_savgol) == 288:
                        # print(len(interp_time_muf))
                        if m < 3:
