...
def detect(
    ...
    above-threshold cells along frequency, and both of its range-neighbours are
    above ``neighbour_threshold`` -- the original's rule.
    """
    ...
    # Both range-neighbours of the anchor must also be lit. Edge columns have
    # only one neighbour and are excluded.
    strong = power > neighbour_threshold
    neighbours = np.zeros_like(mask)
    neighbours[:, 1:-1] = strong[:, :-2] & strong[:, 2:]

    return anchored & neighbours
...
def extract(
    ...
    Args:
        ...
        legacy: reproduce the original decision rule (no continuity test).
    """
    if legacy:
        min_run = 1
